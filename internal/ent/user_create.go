...
func (uc *UserCreate) SetTeam(s string) *UserCreate {
	uc.mutation.SetTeam(s)
	return uc
}

// SetNillableTeam sets the "team" field if the given value is not nil.
func (uc *UserCreate) SetNillableTeam(s *string) *UserCreate {
	if s != nil {
		uc.SetTeam(*s)
	}
	return uc
...
func (uc *UserCreate) AddBattingRecordIDs(ids ...int) *UserCreate {
	uc.mutation.AddBattingRecordIDs(ids...)
	return uc
}

// AddBattingRecords adds the "batting_records" edges to the BatterRecord entity.
func (uc *UserCreate) AddBattingRecords(b ...*BatterRecord) *UserCreate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return uc.AddBattingRecordIDs(ids...)
}

// AddPitchingRecordIDs adds the "pitching_records" edge to the PitcherRecord entity by IDs.
func (uc *UserCreate) AddPitchingRecordIDs(ids ...int) *UserCreate {
	uc.mutation.AddPitchingRecordIDs(ids...)
	return uc
}

// AddPitchingRecords adds the "pitching_records" edges to the PitcherRecord entity.
func (uc *UserCreate) AddPitchingRecords(p ...*PitcherRecord) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddPitchingRecordIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.Team(); ok {
	...
	if nodes := uc.mutation.BattingRecordsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := uc.mutation.PitchingRecordsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
