...
func (tx *Tx) init() {
	tx.BatterRecord = NewBatterRecordClient(tx.config)
	tx.Game = NewGameClient(tx.config)
	tx.PitcherRecord = NewPitcherRecordClient(tx.config)
...
// applies a query, for example: BatterRecord.QueryXXX(), the query will be executed
