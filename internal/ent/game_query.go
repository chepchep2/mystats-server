...
func (gq *GameQuery) QueryBattingRecords() *BatterRecordQuery {
	query := (&BatterRecordClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GameQuery) QueryPitchingRecords() *PitcherRecordQuery {
	query := (&PitcherRecordClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GameQuery) Clone() *GameQuery {
	...
	return &GameQuery{
		...
		withBattingRecords:  gq.withBattingRecords.Clone(),
		withPitchingRecords: gq.withPitchingRecords.Clone(),
...
func (gq *GameQuery) WithBattingRecords(opts ...func(*BatterRecordQuery)) *GameQuery {
	query := (&BatterRecordClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withBattingRecords = query
	return gq
}
...
func (gq *GameQuery) WithPitchingRecords(opts ...func(*PitcherRecordQuery)) *GameQuery {
	query := (&PitcherRecordClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withPitchingRecords = query
	return gq
}
...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	var (
		...
		loadedTypes = [3]bool{
			gq.withUser != nil,
			gq.withBattingRecords != nil,
			gq.withPitchingRecords != nil,
		}
	)
	if gq.withUser != nil {
		withFKs = true
	...
	if query := gq.withBattingRecords; query != nil {
		if err := gq.loadBattingRecords(ctx, query, nodes,
			func(n *Game) { n.Edges.BattingRecords = []*BatterRecord{} },
			func(n *Game, e *BatterRecord) { n.Edges.BattingRecords = append(n.Edges.BattingRecords, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withPitchingRecords; query != nil {
		if err := gq.loadPitchingRecords(ctx, query, nodes,
			func(n *Game) { n.Edges.PitchingRecords = []*PitcherRecord{} },
			func(n *Game, e *PitcherRecord) { n.Edges.PitchingRecords = append(n.Edges.PitchingRecords, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GameQuery) loadBattingRecords(ctx context.Context, query *BatterRecordQuery, nodes []*Game, init func(*Game), assign func(*Game, *BatterRecord)) error {
	...
	query.Where(predicate.BatterRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(game.BattingRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.game_batting_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "game_batting_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "game_batting_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (gq *GameQuery) loadPitchingRecords(ctx context.Context, query *PitcherRecordQuery, nodes []*Game, init func(*Game), assign func(*Game, *PitcherRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Game)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PitcherRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(game.PitchingRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.game_pitching_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "game_pitching_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "game_pitching_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
